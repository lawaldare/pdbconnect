...
export class UploadPageFacade {
  ...
  public ligandCompIds = computed<string[]>(() => {
    const model: any = this.model();
    if (!model?.atomicHierarchy || !model?.entities) return [];

    const entities = model.entities;

    // 1️⃣ Entity subtypes: e.g. ["polypeptide(L)", "other", "other"]
    let subtypeArr: string[] = [];
    if (entities.subtype) {
      subtypeArr = this.colToArray(entities.subtype).map((v) => String(v).toLowerCase());
    } else if (entities.type) {
      subtypeArr = this.colToArray(entities.type).map((v) => String(v).toLowerCase());
    }

    if (!subtypeArr.length) return [];

    // Non-polymer entity indices = ligand entities (ISP, HEM, etc.)
    const ligandEntityIdx = subtypeArr
      .map((t, i) => ({ t, i }))
      .filter((x) => !x.t.includes('polymer') && !x.t.includes('polypeptide'))
      .map((x) => x.i);

    if (!ligandEntityIdx.length) return [];

    // 2️⃣ Walk atomicHierarchy residues → chains → entities
    const ah = model.atomicHierarchy;
    const residues = ah.residues;
    const chains = ah.chains;

    const resToChain = this.colToArray(residues.chainIndex); // residue → chain index
    const chainToEntity = this.colToArray(chains.entityIndex); // chain → entity index
    const compIds = this.colToArray(residues.label_comp_id) // residue → 3-letter code
      .map((v) => String(v).toUpperCase());

    const resCount = compIds.length;
    const ligSet = new Set<string>();
    const skip = new Set(['HOH', 'WAT', 'DOD']); // ignore waters, tweak if you like

    for (let r = 0; r < resCount; r++) {
      const chainIdx = resToChain[r];
      const entityIdx = chainToEntity[chainIdx];
      if (!ligandEntityIdx.includes(entityIdx)) continue;

      const compId = compIds[r];
      if (!compId || skip.has(compId)) continue;

      ligSet.add(compId);
    }

    console.log('subtypeArr', subtypeArr);
    console.log('ligandEntityIdx', ligandEntityIdx);
    console.log('resToChain[0..10]', resToChain.slice(0, 10));
    console.log('chainToEntity', chainToEntity);
    console.log('compIds[0..10]', compIds.slice(0, 10));

    return Array.from(ligSet).sort(); // e.g. ["ISP", "SO4"]
  });
