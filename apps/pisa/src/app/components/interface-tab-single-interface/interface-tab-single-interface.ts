...
export class InterfaceTabSingleInterfaceComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pisaStore
      ...
      .subscribe((response) => {
        ...
        const selectedinterfaceData = this.interfaceTypeData()?.interfaces.find((row: any) => row.interface_id === selectedInterfaceId);
  ...
  public onChangePage(num: number): void {
    ...
    this.selectedInterfaceRow.set(this.interfaceTypeData().interfaces[p]);
    this.pisaStore.dispatch(PisaActions.getInterfaceResultForInterfaceIdForInterfacesTab({ interfaceId: this.interfaceTypeData().interfaces[p].interface_id }));
  }
