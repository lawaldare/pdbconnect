...
export function filterData(viewerData: ViewerData) {
    const filtered = viewerData["toFilter"].filter((eachItem) => {
      if (
        eachItem["interactionType"] === "default" ||
        viewerData["filters"].length === 0
      ) {
        return true;
      }
    ...
    let perAtomSum: any = {};
    let newTotal = 0;
    const newCountDict = filtered.reduce(
      (newDict: { [key: string]: any }, eachItem) => {
        if (!newDict.hasOwnProperty(eachItem["atomName"])) {
          newDict[eachItem["atomName"]] = {};
          perAtomSum[eachItem["atomName"]] = 0;
        }
        if (
          !newDict[eachItem["atomName"]].hasOwnProperty(eachItem["aminoAcid"])
        ) {
          newDict[eachItem["atomName"]][eachItem["aminoAcid"]] = 0;
        }
        newDict[eachItem["atomName"]][eachItem["aminoAcid"]] +=
          eachItem["aminoAcidNumber"];
    ...
    for (
      let i_heatmap = 0;
      i_heatmap < viewerData["heatmap"].length;
      i_heatmap++
    ) {
      ...
      const toDivide =
        viewerData["freqType"] === "Relative" ? perAtomDivide : newTotal;
      let newCount = 0;
      if (newCountDict[element["atomName"]]) {
        newCount = newCountDict[element["atomName"]][element["residue"]!];
        newCount = newCount ? newCount : 0;
      }
      viewerData["heatmap"][i_heatmap]["score"] =
        newCount / Math.max(toDivide, 1);
      viewerData["heatmap"][i_heatmap]["freq"] =
        (newCount / Math.max(toDivide, 1)).toFixed(2);
      viewerData["heatmap"][i_heatmap]["perc"] =
        ((newCount / Math.max(toDivide, 1)) * 100.0).toFixed(2);
...
export function filterRescaleData(viewerData: ViewerData, rescaleType: string): ViewerData {
    ...
    viewerData["originalHeatmap"] = JSON.parse(
      JSON.stringify(viewerData["heatmap"])
    );
...
export function sortAAsByType(viewerData: ViewerData) {
    ...
    viewerData["heatmap"] = viewerData["originalHeatmap"].filter(
      (eachDatum: HotmapLigDatum) => results.indexOf(eachDatum.residue!) > -1
    );
    return viewerData;
}
...
export function sortAAsByIntFreq(viewerData: ViewerData) {
    let freqDict: { [key: string]: number } = {};
...
export function processInitialData(resultIntDataAcc: LigIntCountsDictionary, atomNamesList: string[]) {
    ...
    let generatedData = processAPIData(atomNamesList, resultIntDataAcc);
    let totalInteractions = generatedData["totalInteractions"];
    let toFilter = generatedData["toFilter"];
