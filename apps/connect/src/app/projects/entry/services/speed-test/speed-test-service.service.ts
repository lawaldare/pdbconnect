...
/**
 * Copied from
 * https://github.com/jrquick17/ng-speed-test/blob/4ae0bbd1a3a0a8c231ee49f93fe9e698162d20d6/src/services/speed-test.service.ts
 * Removed checkConnectivity extra calls
 */

@Injectable({
  providedIn: 'root',
})
export class SpeedTestServiceCustom {
  ...
  private mergeSettings(defaultSettings: SpeedTestSettingsModel, customSettings?: Partial<SpeedTestSettingsModel>): SpeedTestSettingsModel {
    ...
    // Merge iterations
    mergedSettings.iterations = customSettings.iterations !== undefined ? customSettings.iterations : defaultSettings.iterations;

    // Merge retryDelay
    mergedSettings.retryDelay = customSettings.retryDelay !== undefined ? customSettings.retryDelay : defaultSettings.retryDelay;
    ...
    if (customSettings.file) {
      ...
      // Merge file path
      mergedSettings.file.path = customSettings.file.path !== undefined ? customSettings.file.path : defaultSettings.file!.path;

      // Merge file size
      mergedSettings.file.size = customSettings.file.size !== undefined ? customSettings.file.size : defaultSettings.file!.size;

      // Merge shouldBustCache
      mergedSettings.file.shouldBustCache =
        customSettings.file.shouldBustCache !== undefined ? customSettings.file.shouldBustCache : defaultSettings.file!.shouldBustCache;
    } else {
  ...
  isOnline(): Observable<boolean> {
    return merge(fromEvent(window, 'offline').pipe(map(() => false)), fromEvent(window, 'online').pipe(map(() => true)), of(navigator.onLine)).pipe(
  ...
  getNetworkStatus(): Observable<{ isOnline: boolean; effectiveType?: string; downlink?: number }> {
    ...
    return merge(
      ...
      fromEvent(window, 'online').pipe(
        ...
        switchMap((info) => this.checkConnectivity().pipe(map((actuallyOnline) => ({ ...info, isOnline: actuallyOnline }))))
      ),
      of(getConnectionInfo()).pipe(
        switchMap((info) => (info.isOnline ? this.checkConnectivity().pipe(map((actuallyOnline) => ({ ...info, isOnline: actuallyOnline }))) : of(info)))
      )
    );
  }
}
