...
export class ValidationDataProcessingFacade {
  ...
  createResidueWiseOutliersMolstar(outliers: ResidueWiseOutliersMolecule[], currentModelIdx: number) {
    const uniqueOutlierTypes = new Set<string>();
    const flattenedResidues: Array<FlatOutlierResidue> = [];

    for (const molecule of outliers) {
      for (const chain of molecule.chains) {
        const model = chain.models.find((m) => m.model_id === currentModelIdx);
        if (!model) continue;

        for (const residue of model.residues) {
          // Collect unique outlier types
          residue.outlier_types.forEach((type) => uniqueOutlierTypes.add(type));
    ...
    // Step 2: For each outlier_type, get residues containing that type
    const residuesByOutlierType: Record<string, typeof flattenedResidues> = {};
    const molstarSelectionsByOutlierType: Record<string, MolstarSelectionObj> = {};

    uniqueOutlierTypes.forEach((type) => {
      residuesByOutlierType[type] = flattenedResidues.filter((residue) => residue.outlier_types.includes(type));
      molstarSelectionsByOutlierType[type] = this.flatOutliersToMolstarSelections(flattenedResidues.filter((residue) => residue.outlier_types.includes(type)));
    });
    ...
    // TODO:
    // Convert into Molstar selection Objs signals

    // Logs
    // console.log(`[Signal] residueWiseOutliers updated for model ${currentModelIdx}`);
    // console.log("Unique Outlier Types:", Array.from(uniqueOutlierTypes));
    // console.log("Residues by Outlier Type:", residuesByOutlierType);
    // console.log("Residues with 1 outlier type:", residuesWith1Outlier);
    // console.log("Residues with 2 outlier types:", residuesWith2Outliers);
    // console.log("Residues with 3 or more outlier types:", residuesWith3OrMoreOutliers);

    return {
      uniqueOutlierTypes: uniqueOutlierTypes,
      molstarSelectionsByOutlierType: molstarSelectionsByOutlierType,
      residuesWith1Outlier: this.flatOutliersToMolstarSelections(residuesWith1Outlier),
      residuesWith2Outliers: this.flatOutliersToMolstarSelections(residuesWith2Outliers),
      residuesWith3OrMoreOutliers: this.flatOutliersToMolstarSelections(residuesWith3OrMoreOutliers),
    };
  }

  flatOutliersToMolstarSelections(flattenedResidues: FlatOutlierResidue[]) {
