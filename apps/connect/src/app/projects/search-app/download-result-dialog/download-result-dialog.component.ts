...
@Component({
    selector: "download-result-dialog",
...
export class DownloadResultDialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.dialogData.selectedEntries.length > 0)
  ...
  onCheckboxChange(e) {
    let selectedFieldIndex = this.selectedFields.indexOf(e.source.value);
    if (e.checked) {
      if (selectedFieldIndex == -1) this.selectedFields.push(e.source.value);
    } else {
      if (selectedFieldIndex > -1)
        this.selectedFields.splice(selectedFieldIndex, 1);
    }
  }

  triggerDownload() {
    let downloadData = this.dialogData.solrQueryData;
    if (typeof downloadData != "undefined") {
      ...
      if (this.downloadFormat == "json" || this.downloadFormat == "xml") {
        ...
        //remove entity_id
        var eIdIndex = selectedFields.indexOf("entity_id");
      ...
      if (this.downloadType == "all") {
        if (
          typeof downloadData.q != "undefined" &&
          downloadData.q != "" &&
          downloadData.q != "q="
        )
          downloadUrl += "&" + downloadData.q;
        if (
          typeof downloadData.fq != "undefined" &&
          downloadData.fq != "" &&
          downloadData.fq != "fq="
        )
          downloadUrl += "&" + downloadData.fq;
        if (
          typeof downloadData.paramCardString != "undefined" &&
          downloadData.paramCardString != ""
        )
          downloadUrl += "&" + downloadData.paramCardString;
      } else {
        downloadUrl +=
          "&q=pdb_id:" + this.dialogData.selectedEntries.join(" OR pdb_id:");
      }
      ...
      this.ajaxSubscriber = this.downloadService
        ...
        .subscribe(
          (res) => {
            ...
            this.downloadService.saveFile(
              res.blob(),
              "PDBe_search." + this.downloadFormat
            );
          },
          (err) => {
            this.downloadInProgress = false; //hide loading
