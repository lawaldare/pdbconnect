...
export class ThorService {
  ...
  constructor(
    ...
    // Thor Hub Project WebService to claim new work
    this.wsAddBatchWorkUrl = appSettings.thorUrl + 'api/dataclaiming/claimWorkBatch';

    // Webservice to get claiming data by pdbIds
    this.wsGetClaimDataByIdsUrl = appSettings.thorUrl + 'api/dataclaiming/findClaimByDataset/PDB';

    // Webservice to get single entry claim data
    this.wsGetSingleClaimData = appSettings.thorUrl + 'api/orcid/find/pdb-self:';
  }
  ...
  buildUrl(base: string | string[], key: string, value: string) {
  ...
  loadClaimingInfo(dbName: string) {
    //set oauthclient
    if (typeof dbName != 'undefined' && dbName != '') this.oauthclient = dbName;

    //build url
    let url = this.buildUrl(this.wsClaimingUrl, 'clientAddress', this.getServerNamePort());
    ...
    return this.http.get(url, options).pipe(
      map((resp: any) => {
        ...
        if (typeof resp['orcIdRecord'] != 'undefined' && resp['orcIdRecord'] != null && resp['orcIdRecord'].works != null && resp['orcIdRecord'].works.length > 0) {
          resp['orcIdRecord'].works.forEach((workRec: { workExternalIdentifiers: string | any[] }) => {
            if (
              typeof workRec.workExternalIdentifiers != 'undefined' &&
              workRec.workExternalIdentifiers.length > 0 &&
              workRec.workExternalIdentifiers[0].workExternalIdentifierType == 'pdb'
            ) {
              this.thorInfo.claimedIds.push(workRec.workExternalIdentifiers[0].workExternalIdentifierId);
            }
          });
        }
        return resp;
  ...
  doBatchClaimRequest() {
    ...
    const dataArr = [];
    for (const dataKey in this.thorInfo.claimRequestData) {
      dataArr.push(this.thorInfo.claimRequestData[dataKey]);
    }

    const body = JSON.stringify({ orcIdWorkLst: dataArr });
  ...
  getLoginUrl(dataUrlValue: any, remember: boolean) {
    let url = dataUrlValue;
  ...
  getClaimDataByIdentifier(identifiers: any[]) {
    const options = {
      headers: new HttpHeaders({
        oauthclient: this.oauthclient,
        Accept: 'application/json',
    ...
    const body = JSON.stringify({ orcIdWorkLst: identifiers });
  ...
  getSingleClaimData(pdbId: string) {
    const options = {
      headers: new HttpHeaders({
        oauthclient: this.oauthclient,
      }),
