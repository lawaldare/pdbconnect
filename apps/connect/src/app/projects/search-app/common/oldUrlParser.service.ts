...
export class OldUrlParserService {
  ...
  parse(urlParams: { keys: any[]; params: { [x: string]: string } }) {
    const searchParamObj: any = {};
    urlParams.keys.forEach((paramDetail) => {
      const paramDetailArr = paramDetail.split(/:(.+)/);

      if (paramDetailArr.length > 1) {
        const paramFieldName =
          typeof this.oldFieldMapping[paramDetailArr[0]] != 'undefined'
            ? this.oldFieldMapping[paramDetailArr[0]]
            : paramDetailArr[0] == 'text'
              ? 'text'
              : 'q_' + paramDetailArr[0];
        ...
        for (let vi = 0, vlen = valArr.length; vi < vlen; vi++) {
          ...
          if (isValidField) {
            ...
            const rangeMatch = valArr[vi].match(/\[(.*)\sTO\s(.*)\]/);
        ...
        if (paramDetail == 'view') {
          const tabMappings = ['entry', 'macromolecules', 'compounds', 'sequencefam'];
          if (typeof searchParamObj['resultState'] == 'undefined') searchParamObj['resultState'] = {};
          const paramTabIndex = tabMappings.indexOf(urlParams.params[paramDetail]);
  ...
  getCleanedValue(paramFieldName: string, paramFieldVal: string, paramFieldData: { value: any; submitFilter: string; type: string }) {
    const cleanSlectedVal: any = {
      value: paramFieldVal,
      condition1: 'AND',
    };
    ...
    if (paramFieldData.submitFilter == 'processAssemblyType') {
      ...
      const rangeMatch = paramFieldVal.match(/\[(.*)\sTO\s(.*)\]/);
