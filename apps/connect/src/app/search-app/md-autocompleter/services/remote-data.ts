...
export class RemoteData extends CompleterBaseData {
  ...
  private _urlFormater!: (term: string) => string;
  private _dataField = '';
  ...
  public search(term: string): void {
    const qFieldMatch: any = this._remoteUrl.match(/facet\.field\=(.*)/);
    ...
    if (this._urlFormater) {
      ...
      url = this._remoteUrl + '&facet.contains.ignoreCase=true&facet.threads:-1&facet.contains=' + encodedTerm + '&q=' + qTerm;
    ...
    this.remoteSearch = this.http
      ...
      .pipe(
        map((data: any) => {
          ...
          let matches = [];
          for (const field in data.facet_counts.facet_fields) {
          ...
          matches.forEach((matchValue: string, i: number) => {
            if (i % 2 === 0) {
              const matchWithoutPipe = matchValue.split('|');
              const escapedVal = term.replace(/([.?*+^$[\]\\(){}|-])/g, '\\$1'); //Bracket issue fix
