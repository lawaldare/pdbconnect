...
const COMPONENT_COLORS: HexColorT[] = [
  ...
  '#e377c2',
  '#bcbd22',
  '#17becf', // More non-conflicting colors from other palettes
  '#fc8d62',
  '#9eb9f3',
  '#ff9da7',
  '#ffff33',
  ...
  '#e15759',
  '#c69fbb',
  '#76b7b2', // More non-conflicting colors from other palettes
];
...
export function pisaComplexView(
  ...
  params: {
    ...
    complexesData: PisaComplexRecord[];
    complexKey: number;
    interfacesData: PisaInterfaceData[];
  }
) {
  ...
  for (const int of interfacesData) {
    ...
    interfaceTooltips[int.interface_id] = `<br>Interface ${int.interface_id}: <b>${moleculeTitle(int.interface.molecules[0], true)}</b> &ndash; <b>${moleculeTitle(
      int.interface.molecules[1],
      true
    )}</b> (interface area ${int.interface.int_area.toFixed(1)})`;
...
export function pisaInterfaceView(
  ...
  params: {
    ...
    structureFormat: ParseFormatT;
    complexesData: PisaComplexRecord[];
    interfaceData: PisaInterfaceData;
    ghostMolecules?: (0 | 1)[];
    detailMolecules?: (0 | 1)[];
  ...
  const interfaceTooltip = `<br>Interface <b>${moleculeTitle(interfaceData.interface.molecules[0], true)}</b> &ndash; <b>${moleculeTitle(
    ...
    true
  )}</b> (interface area ${interfaceData.interface.int_area.toFixed(1)} &angst;<sup>2</sup>)`;
  ...
  return builder.getSnapshot({
    ...
    title: `Interface ${interfaceData.interface_id}: ${moleculeTitle(interfaceData.interface.molecules[0])} - ${moleculeTitle(
      interfaceData.interface.molecules[1]
    )} (interface area ${interfaceData.interface.int_area.toFixed(1)})`,
    linger_duration_ms: 5000,
    transition_duration_ms: 250,
  });
}
...
function markInterface(
  ...
  interfaceSelector: ComponentExpressionT[],
  options: { color?: boolean; elementColors?: boolean; ball_and_stick?: boolean; tooltip?: string; flash?: { ref: string; order: number } } = {}
) {
...
function formatLabelAuth(label: string | number | null, auth: string | number, insCode?: string | null, htmlFormatting?: boolean): string {
  if (!label || (label === auth && !insCode)) {
    ...
    if (htmlFormatting) return `${label} [<small>auth </small>${auth}${insCode ?? ''}]`;
    else return `${label} [auth ${auth}${insCode ?? ''}]`;
  }
}
