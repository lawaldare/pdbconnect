...
export async function patchLigandsSequence() {
  ...
  if (!OriginalSequence) {
    console.warn('⚠️ nightingale-sequence not yet loaded — patchLigandsSequence deferred');
    return;
  }
  ...
  class PatchedLigandsSequence extends (OriginalSequence as any) {
    sequenceData?: Array<string>;
    #seq_bg?: Selection<SVGGElement, any, any, any>;
    #axis?: Selection<SVGGElement, any, any, any>;
    protected seq_g?: Selection<SVGGElement, any, any, any>;
    protected highlighted?: Selection<SVGGElement, any, any, any>;
    margins?: Selection<SVGGElement, any, any, any>;
    #bases?: Selection<SVGTextElement, any, any, any>;
    ...
    connectedCallback() {
      ...
      this.numberOfTicks = Number.isInteger(ticks) ? ticks : DEFAULT_NUMBER_OF_TICKS;
      ...
      const parentEl = this['parentElement'] as HTMLElement | null;
    ...
    protected getCharSize() {
      ...
      const xratio = 0.8;
      const yratio = 1.6;
      const tempNode = this.seq_g.append('text').attr('class', 'base').text('T');
    ...
    renderD3() {
      ...
      if (this.#axis) {
        ...
        const last = Math.ceil(Math.min(this.sequenceData?.length || 0, this['getEnd']()));
        ...
        this.#axis.attr('transform', `translate(${this['margin-left'] + half},${this['margin-top']})`);
        this.#axis.select('.domain').remove();
        this.#axis.selectAll('.tick line').remove();
        this.#axis.selectAll('.tick text').attr('y', 2).attr('font-size', '10px');

        if (this.seq_g) {
          this.seq_g.attr('transform', `translate(0,${this['margin-top'] + 0.75 * this['getHeightWithMargins']()})`);
          ...
          textElements
            ...
            .attr('x', (d) => this['getXFromSeqPosition'](d.position) + half);
          if (this.#seq_bg) {
            const background = this.#seq_bg.selectAll('rect.base_bg').data(bases, (d: any) => d.position);
            background
              .enter()
              .append('rect')
  ...
  customElements.define('nightingale-sequence-patched', PatchedLigandsSequence as unknown as CustomElementConstructor);
}
